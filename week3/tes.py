...
# Date: 09FEB2020
#
# calculate ammount you can barrow based off of 2850 a month downpayment of 75000 and PMI at 1.8
...
def chose_max():
    ...
    monthly_inp = input("Chose the ammount you want to spend monhtly on a house.")
    if monthly_inp == "1":
       monthly_max = monthly_max1
       print("Using $"+str(monthly_max1)+" as monthly payment.")
    elif monthly_inp == "2":
        monthly_max = monthly_max2
        print("Using $"+str(monthly_max2)+" as monthly pyament.")
    else:
...
def calc_mortgage_payment():
    mortgage_payment = can_barrow/((1-(1+mortgage_rate/12)**(-12*30))/(mortgage_rate/12))
...
print("With a down payment of $"+str(down_payment)+" and a monthly max payment of $"+str(monthly_max)+" you can get a loan for $"+str(round(can_barrow - down_payment,2)))
